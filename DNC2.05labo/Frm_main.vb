...
Imports System.Data.Odbc    '200311 oda ロボット/検査モジュール用のファイル作成

' 必要なインポート
Imports System.Runtime.InteropServices
Imports Microsoft.VisualBasic.FileIO    '200120 oda 自動検証機能
...
Public Class Frm_main
    ...
    Public T_exception As DataTable '170921 hishiki 分割文字列内に類似文字が複数存在する場合のエラー対応 exception.csv追加
    ...
    '200423 oda プロセス間通信受信機能 -->
    Public WithEvents PipeClient As New NamedPipeAsyncClient()
    Public IsReceived As Boolean = False
    '200423 oda プロセス間通信受信機能 <--

    Public isVerification As Boolean = False    '200120 oda 自動検証機能

    '200311 oda ロボット/検査モジュール用のファイル作成 -->
    Public ROBOT_txt As String          'Robot.csvのファイル名（絶対アドレス）
    Public INSPECTION_json As String    'Inspection.jsonのファイル名（絶対アドレス）
    Public dicWorkKind As New Dictionary(Of String, String)     '検査モジュール用のコード表：工程判別
    Public dicMaterial As New Dictionary(Of String, String)     '検査モジュール用のコード表：材質
    Public dicSurface As New Dictionary(Of String, String)      '検査モジュール用のコード表：表面処理
    Public tblToleranceShaftClass As New DataTable()            '軸のはめあい記号表
    Public tblToleranceShafts As New DataTable()                '軸のはめあい公差表
    Public tblToleranceLength As New DataTable()                '長さ寸法の普通公差表（中級：m）
    '200311 oda ロボット/検査モジュール用のファイル作成 <--
    ...
    '160808 hishiki 小数点以下桁数設定
    Public DecimalPlace As Integer = 4 'デフォルト値を4とする
    Public NumberOfDigit As Integer = 5 'デフォルト値を5とする

    '161114 hishiki　QRフォーマット変更対応（PO@型式@数量）
    Public QRmode As Integer = 0 'QRmode=0：型式の前の文字列をし定数分だけ削除するモード　QRmode=1：@区切りの
    Public QRSplit As String() 'QR文字列を「@」で分割した文字列の保持用変数
    Public QRspltStr As String = "" 'QR文字列を分割する際の基点となる文字
    Public POplc As Integer = 1 'QR文字列のPOの位置
    Public MODELplc As Integer = 2 'QR文字列の型式の位置
    Public QTYplc As Integer = 3 'QR文字列の数量の位置
    Public QTY_PO As Integer = 0 '指示書の指定数量(デフォルトを0とする)

    '191217 hishiki 八木さん依頼対応
    Public tmpQTY As Integer = 0
    Public tmpMODEL As String = ""
    ...
    Private Sub Frm_main_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            '200311 oda ロボット/検査モジュール用のファイル作成 -->
            LoadDbToTBL("TOLERANCE_FITTING_SHAFT_CLASS", tblToleranceShaftClass)
            LoadDbToTBL("TOLERANCE_FITTING_SHAFTS", tblToleranceShafts)
            LoadDbToTBL("TOLERANCE_GENERAL_LENGTH", tblToleranceLength)
            '200311 oda ロボット/検査モジュール用のファイル作成 -->
            ...
            For Each rw As DataRow In DSbase.Tables("T_prg").Rows
                ...
                T_exception = New DataTable("T_exception" & prgNo) '170921 hishiki 分割文字列内に類似文字が複数存在する場合のエラー対応
                ...
                LoadFileToTBL(System.IO.Path.Combine(Path, folder), "exception.csv", DSprg, T_exception) '170921 hishiki 分割文字列内に類似文字が複数存在する場合のエラー対応

                '200311 oda ロボット/検査モジュール用のファイル作成 -->
                LoadFileToDic(IO.Path.Combine(Path, folder), "work_kind.csv", dicWorkKind)
                LoadFileToDic(IO.Path.Combine(Path, folder), "material.csv", dicMaterial)
                LoadFileToDic(IO.Path.Combine(Path, folder), "surface.csv", dicSurface)
                '200311 oda ロボット/検査モジュール用のファイル作成 -->
            Next
            ...
            'プロセス間通信受信開始
            PipeClient.Open()   '200423 oda プロセス間通信受信機能
    ...
    '200311 oda 検査モジュール用のファイル作成 -->
    Private Sub LoadFileToDic(folder As String, fileName As String, ByRef dic As Dictionary(Of String, String))

        Dim master_path = System.IO.Path.Combine(folder, fileName)
        Dim encoding As System.Text.Encoding = System.Text.Encoding.Default

        Try
            Using parser As New TextFieldParser(master_path, encoding)
                With parser
                    ' カンマ区切りの指定
                    ...
                    ' ファイルの終端までループ（ヘッダー含む）
                    While Not .EndOfData
                        Dim fields As String() = .ReadFields()
                        dic.Add(fields(0), fields(1))
                    End While
                End With
            End Using
    ...
    Private Sub LoadDbToTBL(tableName As String, ByRef dt As DataTable)

        Dim lstDoubleField As New List(Of String)(New String() {"GT", "LE", "UPPER_LIMIT", "LOWER_LIMIT"})

        Dim master_path = System.IO.Path.Combine(Me.Path, tableName + ".csv")
        Dim encoding As System.Text.Encoding = System.Text.Encoding.Default
        ...
        Try
            Using parser As New TextFieldParser(master_path, encoding)
                With parser
                    ...
                    Dim data As String() : Dim i As Integer

                    ' データテーブルのフィールドを設定
                    If Not parser.EndOfData Then
                        data = parser.ReadFields()  'ヘッダ行

                        Dim cols As Integer = data.Length

                        For i = 0 To cols - 1
                            If lstDoubleField.Contains(data(i)) Then
                                dt.Columns.Add(New DataColumn(data(i), System.Type.GetType("System.Double")))
                            Else
                    ...
                    ' ファイルの終端までループ
                    While Not .EndOfData
                        data = parser.ReadFields()  'データ行

                        Dim row As DataRow = dt.NewRow()

                        For i = 0 To dt.Columns.Count - 1
                            If data(i).Length.Equals(0) Then
                                row(i) = DBNull.Value
                            Else
                                row(i) = data(i)
                            End If
    ...
    '    Dim provider As String = "", datasource As String = ""

    '    Try
    '        Call FindDBProvider(provider, datasource)
    ...
    Private Function GetShaftsTolerance(class_id As String, size As Double, ByRef upper As Double, ByRef lower As Double) As Boolean
        Try
            Dim rows As DataRow(), row As DataRow
            Dim class_code As String

            rows = tblToleranceShaftClass.Select("CLASS_ID = '" & class_id & "'")
            row = rows(0)
            class_code = row("CLASS_CODE").ToString()

            Dim dv As DataView = New DataView(tblToleranceShafts)
            Dim dt As DataTable

            dv.RowFilter = String.Format("GT < {0} AND LE >= {0} AND CLASS_CODE = '{1}'", size, class_code)
            dt = dv.ToTable()

            row = dt.Rows(0)
            upper = row("UPPER_LIMIT") / 1000
            lower = row("LOWER_LIMIT") / 1000
    ...
    Private Function GetLengthTolerance(size As Double, ByRef upper As Double, ByRef lower As Double) As Boolean
        Try
            Dim row As DataRow
            Dim class_code As String = "m"

            Dim dv As DataView = New DataView(tblToleranceLength)
            Dim dt As DataTable
    ...
    'Private Function FindDBProvider(ByRef provider As String, ByRef datasource As String) As Boolean

    '    Dim roots As New List(Of String)
    '    Using reader As System.Data.OleDb.OleDbDataReader =
    ...
    '    Dim DBProviders As New List(Of String) From {"Microsoft.ACE.OLEDB.16.0", "Microsoft.ACE.OLEDB.15.0", "Microsoft.ACE.OLEDB.12.0", "Microsoft.Jet.OLEDB.4.0"}

    '    For Each s As String In DBProviders
    '        If roots.IndexOf(s) >= 0 Then
    '            provider = s
    '            If provider.Equals("Microsoft.Jet.OLEDB.4.0") Then
    '                datasource = System.IO.Path.Combine(Me.Path, "tolerance.mdb")
    '            Else
    '                datasource = System.IO.Path.Combine(Me.Path, "tolerance.accdb")
    '            End If
    '            Return True
    '        End If
    '    Next
...
#Region "プロバイダー確認"
    'Function IsExistOleDbProvider(ByVal sources_name As String) As Boolean
    '    Using reader As System.Data.OleDb.OleDbDataReader =
    '                    System.Data.OleDb.OleDbEnumerator.GetRootEnumerator()
    '        Do While reader.Read()
    '            Dim str = reader.GetString(0)
    '            If reader.GetString(0) = sources_name Then
    '                Return True
    '            End If
    '        Loop
    '        Return False
...
#End Region
        ...
        For i As Integer = 0 To UBound(rw_correct)
            ...
            Else
                ...
                If cnt = 0 Then
                    ...
                    '161114 hishiki start
                    If defaultValue.ToLower = "qty" Then
                        defaultValue = QTY_PO
                    End If
                    '161114 hishiki end
        ...
        Try
            ...
            For Each rw As DataRow In DSbase.Tables("T_ini").Rows
                ...
                ElseIf rw.Item("DEFINE") = "macrotxt" Then
                    ...
                    '200311 oda ロボット/検査モジュール用のファイル作成 -->
                ElseIf rw.Item("DEFINE") = "robot_csv" Then
                    If value <> "" Then
                        ROBOT_txt = value
                    Else
                        ROBOT_txt = "C:\macro\Robot.csv"
                    End If

                ElseIf rw.Item("DEFINE") = "inspection_json" Then
                    If value <> "" Then
                        INSPECTION_json = value
                    Else
                        INSPECTION_json = "C:\macro\Inspection.json"
                    End If
                    '200311 oda ロボット/検査モジュール用のファイル作成 <--
                ...
                ElseIf define = "SmacroBrother" Then '151012 hishiki Brother機能追加
                    ...
                    End If
                    '160808 hishiki 小数点以下桁数をini.csvで可変にできるように修正
                ElseIf define = "DecimalPlace" Then
                    ...
                    End If
                    '160808 hishiki input系の整数部の桁数をini.csvで可変にできるように修正
                ElseIf define = "NumberOfDigit" Then
                    ...
                    '161114 hishiki start
                ElseIf define = "QRmode" Then
                    If IsNumeric(value) Then
                        If value > 0 Then
                            QRmode = value
                        End If
                    Else
                        QRmode = 0 'デフォルト0にしておく
                    End If

                ElseIf define = "POplc" Then
                    If IsNumeric(value) Then
                        If value > 0 Then
                            POplc = value
                        End If
                ...
                ElseIf define = "MODELplc" Then
                    If IsNumeric(value) Then
                        If value > 0 Then
                            MODELplc = value
                        End If
                    Else
                        MODELplc = 2 'デフォルト2にしておく
                    End If

                ElseIf define = "QTYplc" Then
                    If IsNumeric(value) Then
                        If value > 0 Then
                            QTYplc = value
                        End If
                    Else
                        QTYplc = 3 'デフォルト3にしておく
                    End If
                ElseIf define = "QRspltStr" Then
                    If value <> "" Then
                        QRspltStr = value
                    Else
                        QRspltStr = "@" 'デフォルト@にしておく
    ...
    Private Sub TB_Barcode_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles TB_Barcode.KeyDown
        ...
        If (IsReceived) OrElse (e.KeyCode = Keys.Enter) Then
            IsReceived = False
            '2000423 oda プロセス間通信受信機能 <--

            '200120 oda 自動検証機能 -->
            If TB_Barcode.Text.Equals("TEST") Or TB_Barcode.Text.Equals("test") Then
                Me.SuspendLayout()
                Call Verification()
                Me.ResumeLayout()
            End If
            ...
            Else
                ...
                Else '型式の場合（ヘッダー付き）
                    ...
                    If QRmode = 1 Then
                        ...
                        QRSplit = Split(TB_Barcode.Text, QRspltStr)
                        If QRSplit.Length > 1 Then
                            For i As Integer = 0 To UBound(QRSplit)
                                If i + 1 = MODELplc Then
                                    modelName = QRSplit(i)
                                ElseIf i + 1 = QTYplc Then
                                    If IsNumeric(QRSplit(i)) Then
                                        QTY_PO = QRSplit(i)
                                    End If
                                End If
                            Next
                        ...
                        '191217 hishiki 八木さん依頼対応 start
                        tmpQTY = QTY_PO
                        tmpMODEL = modelName
                        '191217 hishiki 八木さん依頼対応 end
                ...
                If modelName <> "" Then
                    Me.SuspendLayout()  '200120 oda コントロールレイアウト前、画面描画一時停止
                    SearchTB_Model(modelName) '型式をマスタから検索して画面に表示
                    Me.ResumeLayout()   '200120 oda コントロールレイアウト後、画面描画再開
                End If
    ...
    Public Sub SearchTB_Model(ByVal modelName As String)
        ...
        '200120 oda 自動検証機能 -->
        If modelName.Equals("TEST") Or modelName.Equals("test") Then
            Me.SuspendLayout()
            Call Verification()
            Me.ResumeLayout()
        End If
    ...
    Private Function getValueFrmMaster(ByVal define As String, ByVal macro As String, ByVal grpNo As Integer) As String
        ...
        ElseIf define.IndexOf("define") = 0 Then
            ...
            Else
                ...
                '170921 hishiki start 分割文字列内に類似文字が複数存在する場合のエラー対応
                Dim exceptModelSplit As String() = searchDefineFrmTexclusion(define, modelSplit)
                tmpStr = searchDefineFrmRows(targetStr, rws, exceptModelSplit)
                'tmpStr = searchDefineFrmRows(targetStr, rws, modelSplit)
                '170921 hishiki end 分割文字列内に類似文字が複数存在する場合のエラー対応
    ...
    '170921 hishiki 分割文字列内に類似文字が複数存在する場合のエラー対応
    'TODO excepton.csvが存在しない場合の処理実装
    Private Function searchDefineFrmTexclusion(ByVal define As String, ByVal strs As String()) As String()

        Dim rtnStr() As String
        ReDim rtnStr(0)

        'exception.csvが存在しない場合、引数の文字列をそのまま返却
        If IsNothing(DSprg.Tables("T_exception" & dispPrgNo)) Then
            Return strs
            Exit Function
        End If

        '除外マスタを検索。DEFINEに項目が無い場合は、引数の文字列をそのまま返却
        Dim exRws As DataRow() = DSprg.Tables("T_exception" & dispPrgNo).Select("DEFINE='" & define & "'")
        If exRws.Length = 0 Then
            Return strs
            Exit Function
        End If

        '除外マスタにヒットした場合、除外対象文字列を除いた文字列を返却
        Dim cnt As Integer = 0
        For i As Integer = 0 To UBound(strs)
            Dim tmpStr As String = strs(i)
            Dim flg_except As Integer = 0
            For Each rw As DataRow In exRws
                Dim exStr As String = rw.Item("VALUE").ToString

                '除外文字列にヒットした場合、除外フラグをたてる
                If tmpStr.StartsWith(exStr) Then
                    flg_except = 1
                    Exit For
                End If
            Next

            '除外フラグがたっていない場合返却文字列へセット
            If flg_except = 0 Then
                ReDim Preserve rtnStr(cnt)
                rtnStr(cnt) = tmpStr
                cnt = cnt + 1
            End If
        Next
        ...
        If c.Length = 1 Then
            ...
            Else
                ...
                Else
                    ...
                    If DSprg.Tables("T_cntrl" & dispPrgNo).Columns.Contains("MAX") And
    ...
    Private Sub makeSendTxt()
        ...
        '200311 oda ロボット/検査モジュール用のファイル作成 -->
        Dim OutputOfRobot As New Dictionary(Of String, String)
        OutputOfRobot.Add("Model", Me.TB_Model.Text)

        Dim OutputOfInspection As New InspectionData(Me.TB_Model.Text)
        Dim feature1 As New Feature
        Dim feature2 As New Feature
        Dim feature3 As New Feature
        Dim feature4 As New Feature
        Dim feature5 As New Feature
        Dim feature6 As New Feature
        '200311 oda ロボット/検査モジュール用のファイル作成 <--
            ...
            Else
                ...
                If sendFlg = "2" Then
                    ...
                    Else 'エラーではない場合
                        ...
                        Select Case macroName
                            Case "3001" : OutputOfRobot.Add("Full L", value)
                            Case "3002" : OutputOfRobot.Add("L", value)
                            Case "3003" : OutputOfRobot.Add("PDmax", value)
                            Case "3004" : OutputOfRobot.Add("Qty", value)
                            Case "3005" : OutputOfRobot.Add("P", value)
                            Case "3006" : OutputOfRobot.Add("D", value)
                            Case "3007" : OutputOfRobot.Add("C", value)
                            Case "3008" : OutputOfRobot.Add("Mizo", value)
                        End Select
                    End If
                End If
                ...
                If sendFlg = "3" Then
                    ...
                    Else 'エラーではない場合
                        ...
                        Dim diameter1 As Double, diameter2 As Double, diameter As Double, length1 As Double, LB As Double
                        Dim class_id As String, upper As Double, lower As Double

                        Select Case macroName
#Region " Header ---------------------------------------------------------- "
                            Case "4100"
                                If dicWorkKind.ContainsKey(value) Then
                                    OutputOfInspection.Data.Work_Kind = dicWorkKind(value)
                                End If
                            Case "4103"
                                OutputOfInspection.Data.Order_qty = value
                            Case "4104"
                                If dicMaterial.ContainsKey(value) Then
                                    OutputOfInspection.Data.Material = dicMaterial(value)
                                End If
                            Case "4105"
                                If dicSurface.ContainsKey(value) Then
                                    OutputOfInspection.Data.Surface_Finish = dicSurface(value)
                                End If
                            Case "4106"
                                If value.Equals("0") Then
                                    OutputOfInspection.Data.Heat_Treatment = "0"
                                Else
                                    OutputOfInspection.Data.Heat_Treatment = "1"
                                End If
                            Case "4107"
                                OutputOfInspection.Data.Insertion_Guide = value
                            Case "4108"
                                OutputOfInspection.Data.Outer_Diameter = value
                            Case "4109"
                                OutputOfInspection.Data.Full_Length = value
#End Region
#Region " Feature1 ---------------------------------------------------------- "
                            Case "4201"
                                feature1.Parameters.Add(New Parameter("ID", value))
                            Case "4202"
                                feature1.Parameters.Add(New Parameter("Diameter1", value))
                                If Not Double.TryParse(value, diameter1) Then diameter1 = 0
                            Case "4203"
                                If value.IndexOf("9991") < 0 Then
                                    feature1.Parameters.Add(New Parameter("Diameter1_Upper", value))
                                Else
                                    class_id = value.Substring(4, 2).TrimStart("0")

                                    If GetShaftsTolerance(class_id, diameter1, upper, lower) Then
                                        feature1.Parameters.Add(New Parameter("Diameter1_Upper", upper))
                                    Else
                                        feature1.Parameters.Add(New Parameter("Diameter1_Upper", "Error"))
                                    End If
                                End If
                            Case "4204"
                                If value.IndexOf("9992") < 0 Then
                                    feature1.Parameters.Add(New Parameter("Diameter1_Lower", value))
                                Else
                                    class_id = value.Substring(4, 2).TrimStart("0")

                                    If GetShaftsTolerance(class_id, diameter1, upper, lower) Then
                                        feature1.Parameters.Add(New Parameter("Diameter1_Lower", lower))
                                    Else
                                        feature1.Parameters.Add(New Parameter("Diameter1_Lower", "Error"))
                                    End If
                                End If
                            Case "4205"
                                feature1.Parameters.Add(New Parameter("Length1", value))
                                If Not Double.TryParse(value, length1) Then length1 = 0
                            Case "4206"
                                If value.IndexOf("9991") < 0 Then
                                    feature1.Parameters.Add(New Parameter("Length1_Upper", value))
                                Else
                                    If GetLengthTolerance(length1, upper, lower) Then
                                        feature1.Parameters.Add(New Parameter("Length1_Upper", upper))
                                    Else
                                        feature1.Parameters.Add(New Parameter("Length1_Upper", "Error"))
                                    End If
                                End If
                            Case "4207"
                                If value.IndexOf("9992") < 0 Then
                                    feature1.Parameters.Add(New Parameter("Length1_Lower", value))
                                Else
                                    If GetLengthTolerance(length1, upper, lower) Then
                                        feature1.Parameters.Add(New Parameter("Length1_Lower", lower))
                                    Else
                                        feature1.Parameters.Add(New Parameter("Length1_Lower", "Error"))
                                    End If
                                End If
                            Case "4208"
                                feature1.Parameters.Add(New Parameter("Chamfer", value))
                            Case "4209"
                                feature1.Parameters.Add(New Parameter("R", value))
                            Case "4210"
                                feature1.Parameters.Add(New Parameter("Groove_Diameter", value))
                            Case "4211"
                                feature1.Parameters.Add(New Parameter("Groove_Width", value))
                            Case "4212"
                                feature1.Parameters.Add(New Parameter("Diameter2", value))
                                If Not Double.TryParse(value, diameter2) Then diameter2 = 0
                            Case "4213"
                                If value.IndexOf("9991") < 0 Then
                                    feature1.Parameters.Add(New Parameter("Diameter2_Upper", value))
                                Else
                                    class_id = value.Substring(4, 2).TrimStart("0")

                                    If GetShaftsTolerance(class_id, diameter2, upper, lower) Then
                                        feature1.Parameters.Add(New Parameter("Diameter2_Upper", upper))
                                    Else
                                        feature1.Parameters.Add(New Parameter("Diameter2_Upper", "Error"))
                                    End If
                                End If
                            Case "4214"
                                If value.IndexOf("9992") < 0 Then
                                    feature1.Parameters.Add(New Parameter("Diameter2_Lower", value))
                                Else
                                    class_id = value.Substring(4, 2).TrimStart("0")

                                    If GetShaftsTolerance(class_id, diameter2, upper, lower) Then
                                        feature1.Parameters.Add(New Parameter("Diameter2_Lower", lower))
                                    Else
                                        feature1.Parameters.Add(New Parameter("Diameter2_Lower", "Error"))
                                    End If
                                End If
                            Case "4215"
                                feature1.Parameters.Add(New Parameter("Length2", value))
                            Case "4216"
                                feature1.Parameters.Add(New Parameter("Length2_Upper", value))
                            Case "4217"
                                feature1.Parameters.Add(New Parameter("Length2_Lower", value))
                            Case "4218"
                                feature1.Parameters.Add(New Parameter("Taper_Length", value))
                            Case "4219"
                                feature1.Parameters.Add(New Parameter("Taper_Angle", value))
                            Case "4220"
                                feature1.Parameters.Add(New Parameter("Taper_Tip_Diameter", value))
                            Case "4221"
                                feature1.Parameters.Add(New Parameter("TipR_Length", value))
                            Case "4222"
                                feature1.Parameters.Add(New Parameter("TipR", value))
                            Case "4223"
                                feature1.Parameters.Add(New Parameter("Chamfer_WithoutInstruction", value))
                            Case "4224"
                                feature1.Parameters.Add(New Parameter("LB", value))
                                If Not Double.TryParse(value, LB) Then LB = 0
                            Case "4225"
                                If value.IndexOf("9991") < 0 Then
                                    feature1.Parameters.Add(New Parameter("LB_Upper", value))
                                Else
                                    If GetLengthTolerance(LB, upper, lower) Then
                                        feature1.Parameters.Add(New Parameter("LB_Upper", upper))
                                    Else
                                        feature1.Parameters.Add(New Parameter("LB_Upper", "Error"))
                                    End If
                                End If
                            Case "4226"
                                If value.IndexOf("9992") < 0 Then
                                    feature1.Parameters.Add(New Parameter("LB_Lower", value))
                                Else
                                    If GetLengthTolerance(LB, upper, lower) Then
                                        feature1.Parameters.Add(New Parameter("LB_Lower", lower))
                                    Else
                                        feature1.Parameters.Add(New Parameter("LB_Lower", "Error"))
                                    End If
                                End If
#End Region
#Region " Feature2 ---------------------------------------------------------- "
                            Case "4301"
                                feature2.Parameters.Add(New Parameter("ID", value, True))
                            Case "4302"
                                feature2.Parameters.Add(New Parameter("Diameter1", value))
                                If Not Double.TryParse(value, diameter1) Then diameter1 = 0
                            Case "4303"
                                If value.IndexOf("9991") < 0 Then
                                    feature2.Parameters.Add(New Parameter("Diameter1_Upper", value))
                                Else
                                    class_id = value.Substring(4, 2).TrimStart("0")

                                    If GetShaftsTolerance(class_id, diameter1, upper, lower) Then
                                        feature2.Parameters.Add(New Parameter("Diameter1_Upper", upper))
                                    Else
                                        feature2.Parameters.Add(New Parameter("Diameter1_Upper", "Error"))
                                    End If
                                End If
                            Case "4304"
                                If value.IndexOf("9992") < 0 Then
                                    feature2.Parameters.Add(New Parameter("Diameter1_Lower", value))
                                Else
                                    class_id = value.Substring(4, 2).TrimStart("0")

                                    If GetShaftsTolerance(class_id, diameter1, upper, lower) Then
                                        feature2.Parameters.Add(New Parameter("Diameter1_Lower", lower))
                                    Else
                                        feature2.Parameters.Add(New Parameter("Diameter1_Lower", "Error"))
                                    End If
                                End If
                            Case "4305"
                                feature2.Parameters.Add(New Parameter("Length1", value))
                            Case "4306"
                                feature2.Parameters.Add(New Parameter("Length1_Upper", value))
                            Case "4307"
                                feature2.Parameters.Add(New Parameter("Length1_Lower", value))
                            Case "4308"
                                feature2.Parameters.Add(New Parameter("R", value))
                            Case "4309"
                                feature2.Parameters.Add(New Parameter("Groove_Diameter", value))
                            Case "4310"
                                feature2.Parameters.Add(New Parameter("Groove_Width", value))
                            Case "4311"
                                feature2.Parameters.Add(New Parameter("Diameter2", value))
                            Case "4312"
                                feature2.Parameters.Add(New Parameter("Taper_Length", value))
                            Case "4313"
                                feature2.Parameters.Add(New Parameter("Taper_Angle", value))
                            Case "4314"
                                feature2.Parameters.Add(New Parameter("Taper_Tip_Diameter", value))
                            Case "4315"
                                feature2.Parameters.Add(New Parameter("TipR_Length", value))
                            Case "4316"
                                feature2.Parameters.Add(New Parameter("TipR", value))
                            Case "4317"
                                feature2.Parameters.Add(New Parameter("Chamfer1_WithoutInstruction", value))
                            Case "4318"
                                feature2.Parameters.Add(New Parameter("Chamfer2_WithoutInstruction", value))
                            Case "4319"
                                feature2.Parameters.Add(New Parameter("Left_Length", value))
#End Region
#Region " Feature3 ---------------------------------------------------------- "
                            Case "4401"
                                feature3.Parameters.Add(New Parameter("ID", value, True))
                            Case "4402"
                                feature3.Parameters.Add(New Parameter("Diameter", value))
                                If Not Double.TryParse(value, diameter) Then diameter = 0
                            Case "4403"
                                If value.IndexOf("9991") < 0 Then
                                    feature3.Parameters.Add(New Parameter("Diameter_Upper", value))
                                Else
                                    class_id = value.Substring(4, 2).TrimStart("0")

                                    If GetShaftsTolerance(class_id, diameter, upper, lower) Then
                                        feature3.Parameters.Add(New Parameter("Diameter_Upper", upper))
                                    Else
                                        feature3.Parameters.Add(New Parameter("Diameter_Upper", "Error"))
                                    End If
                                End If
                            Case "4404"
                                If value.IndexOf("9992") < 0 Then
                                    feature3.Parameters.Add(New Parameter("Diameter_Lower", value))
                                Else
                                    class_id = value.Substring(4, 2).TrimStart("0")

                                    If GetShaftsTolerance(class_id, diameter, upper, lower) Then
                                        feature3.Parameters.Add(New Parameter("Diameter_Lower", lower))
                                    Else
                                        feature3.Parameters.Add(New Parameter("Diameter_Lower", "Error"))
                                    End If
                                End If
                            Case "4405"
                                feature3.Parameters.Add(New Parameter("Length", value))
                            Case "4406"
                                feature3.Parameters.Add(New Parameter("Length_Upper", value))
                            Case "4407"
                                feature3.Parameters.Add(New Parameter("Length_Lower", value))
                            Case "4408"
                                feature3.Parameters.Add(New Parameter("R", value))
                            Case "4409"
                                feature3.Parameters.Add(New Parameter("Groove_Diameter", value))
                            Case "4410"
                                feature3.Parameters.Add(New Parameter("Groove_Width", value))
                            Case "4411"
                                feature3.Parameters.Add(New Parameter("Taper_Length", value))
                            Case "4412"
                                feature3.Parameters.Add(New Parameter("Taper_Angle", value))
                            Case "4413"
                                feature3.Parameters.Add(New Parameter("Taper_Tip_Diameter", value))
                            Case "4414"
                                feature3.Parameters.Add(New Parameter("TipR_Length", value))
                            Case "4415"
                                feature3.Parameters.Add(New Parameter("TipR", value))
                            Case "4416"
                                feature3.Parameters.Add(New Parameter("Chamfer_WithoutInstruction", value))
                            Case "4417"
                                feature3.Parameters.Add(New Parameter("Left_Length", value))
#End Region
#Region " Feature4 ---------------------------------------------------------- "
                            Case "4501"
                                feature4.Parameters.Add(New Parameter("ID", value))
                            Case "4502"
                                feature4.Parameters.Add(New Parameter("Center_Plane_Length", value))
                            Case "4503"
                                feature4.Parameters.Add(New Parameter("Plane_Count", value))
                            Case "4504"
                                feature4.Parameters.Add(New Parameter("Plane_Length", value))
#End Region
#Region " Feature5 ---------------------------------------------------------- "
                            Case "4601"
                                feature5.Parameters.Add(New Parameter("ID", value))
                            Case "4602"
                                feature5.Parameters.Add(New Parameter("Screw_Size", value))
                            Case "4603"
                                feature5.Parameters.Add(New Parameter("Effective_Length", value))
                            Case "4604"
                                feature5.Parameters.Add(New Parameter("PilotHole_Depth", value))
#End Region
#Region " Feature6 ---------------------------------------------------------- "
                            Case "4701"
                                feature6.Parameters.Add(New Parameter("ID", value))
#End Region
#Region " MeasurementMargin ------------------------------------------------------------ "
                            Case "4801"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MW", value))
                            Case "4802"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("DIA_MW", value))
                            Case "4803"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MASK_MGN", value))
                            Case "4804"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN01", value))
                            Case "4805"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN02", value))
                            Case "4806"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN03", value))
                            Case "4807"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN04", value))
                            Case "4808"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN05", value))
                            Case "4809"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN06", value))
                            Case "4810"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN07", value))
                            Case "4811"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN08", value))
                            Case "4812"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN09", value))
                            Case "4813"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN10", value))
                            Case "4814"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN11", value))
                            Case "4815"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("MGN12", value))
                            Case "4816"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("G_MGN01", value))
                            Case "4817"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("G_MGN02", value))
                            Case "4818"
                                OutputOfInspection.Data.MeasurementMargin.Add(New Parameter("G_MGN03", value))
#End Region
                        End Select
                    End If
                End If
        ...
        If errflg = 0 Then
            ...
            '200311 oda ロボット/検査モジュール用のファイル作成 -->
            Dim encoding As System.Text.Encoding = System.Text.Encoding.Default
            Dim output_path As String = ROBOT_txt
            Dim append As Boolean = False

#If DEBUG Then
            output_path = "c:\macro\robot_e500.txt"
#Else
            output_path = ROBOT_txt
#End If
            'ロボット用ファイル出力
            OutputOfRobot.Add("Shizumi_Neji", "0")
            Try
                If OutputOfRobot.Count <> 0 Then
                    Using sw As New StreamWriter(output_path, append, encoding)

                        For Each fields As KeyValuePair(Of String, String) In OutputOfRobot
                            sw.WriteLine(String.Join(" = ", fields.Key, fields.Value))
                        Next
                    End Using
                End If
            Catch ex As Exception
                MsgBox("ロボット用ファイル出力に失敗しました。" & Environment.NewLine & ex.Message)
            End Try

            '検査モジュール用ファイル出力
            OutputOfInspection.Data.FeatureRecognitionResult.Features.Add(feature1)
            OutputOfInspection.Data.FeatureRecognitionResult.Features.Add(feature2)
            OutputOfInspection.Data.FeatureRecognitionResult.Features.Add(feature3)
            OutputOfInspection.Data.FeatureRecognitionResult.Features.Add(feature4)
            OutputOfInspection.Data.FeatureRecognitionResult.Features.Add(feature5)
            OutputOfInspection.Data.FeatureRecognitionResult.Features.Add(feature6)

#If DEBUG Then
            output_path = String.Format("c:\macro\検査Mデバッグ\Fin_{0}[{1}]_FR.json", OutputOfInspection.Data.Part_Number, OutputOfInspection.Data.Feature_Joined_ID)
#Else
            output_path = INSPECTION_json
#End If
            Try
                If OutputOfInspection.Data.MeasurementMargin.Count <> 0 Then
                    Using sw As New StreamWriter(output_path, append, encoding)

                        sw.WriteLine(OutputOfInspection.GetJsonSerial(Newtonsoft.Json.Formatting.Indented))
                    End Using
                End If
            Catch ex As Exception
                MsgBox("検査M用ファイル出力に失敗しました。" & Environment.NewLine & ex.Message)
            End Try
            '200311 oda ロボット/検査モジュール用のファイル作成 <--
    ...
    Private Sub setTBalart()
        ...
        TB_alart.Location = New Point(
            Me.Location.X + (Me.Width - TB_alart.Width) \ 2,
...
#If DEBUG Then
#Else
        If isVerification Then Exit Sub
#End If
        '200120 oda 自動検証機能 <--
        ...
        Try
            ...
            If Cnt > 0 Then
                ...
                '191217 hishiki 八木さんの依頼でラボ用に読み込んだ型式と数量をmodel.txtへ出力（上書き） start
                Dim modelTxt As String = System.IO.Path.GetDirectoryName(MACRO_txt)
                modelTxt = System.IO.Path.Combine(modelTxt, "model.txt")
                'If System.IO.File.Exists(modelTxt) = False Then 'ファイルが存在しない場合新規作成
                '    Dim ParaTemp As New System.IO.StreamWriter(modelTxt, False, System.Text.Encoding.Default)
                '    Dim tmp As String = ""
                '    tmp = tmpMODEL & "," & tmpQTY & vbLf
                '    ParaTemp.WriteLine(tmp)
                '    ParaTemp.Close()
                'Else 'ファイルがあれば開いて上書き
                '    Dim sw As New System.IO.StreamWriter("C:\test\1.txt", True, System.Text.Encoding.Default)
                'End If

                '200318 oda model.txt 出力処理修正（データ追記→上書き、エラー時のメモリーリーク対応）-->
                'Dim ParaTemp As New System.IO.StreamWriter(modelTxt, True, System.Text.Encoding.Default)
                'Dim tmp As String = ""
                'tmp = tmpMODEL & "," & tmpQTY & vbLf
                'ParaTemp.WriteLine(tmp)
                'ParaTemp.Close()

                Using ParaTemp As New System.IO.StreamWriter(modelTxt, False, System.Text.Encoding.Default)
                    Dim tmp As String = ""
                    tmp = tmpMODEL & "," & tmpQTY
                    ParaTemp.WriteLine(tmp)
                End Using
                '200318 oda model.txt 出力処理修正（データ追記→上書き、エラー時のメモリーリーク対応）<--
                '191217 hishiki 八木さんの依頼でラボ用に読み込んだ型式と数量をmodel.txtへ出力（上書き） end
    ...
    '●文字列から計算結果を返す関数。エラーの場合"NG"を返却 180808 hishiki start
    ...
    '160808 hishiki double→decimalへ変更
    Private Function getCalcMathResult(ByVal exp As String, ByVal value As Decimal) As Decimal
        'Private Function getCalcMathResult(ByVal exp As String, ByVal value As Double) As Double

        Dim rtnValue As Decimal
        '    Dim rtnValue As Double

        '160808 hishiki 大文字小文字区別なくすために、epxを小文字へ変換。caseの条件値も小文字へ
        exp = exp.ToLower
        Select Case exp
            Case "abs"
    ...
    '160808 hishiki start
    Private Function getNumericValue(ByVal exp As String) As Decimal
    ...
    Private Function makeFormatStr(ByVal str As String) As String
        ...
        Else
            ...
            For i As Integer = 0 To formatCnt - 1
                ...
                '160808 hishiki start 小数点以下桁数をini.csvで可変にできるように修正
...
#Region "自動検証機能"
    ...
    Private Sub Verification()
        ...
        Const MASTER_FILENAME As String = "verification.csv"
        Const OUTPUT_FILENAME As String = "verification_result.csv"

        Dim master_path As String = System.IO.Path.Combine(Me.Path, MASTER_FILENAME)
        Dim encoding As System.Text.Encoding = System.Text.Encoding.Default
            ...
            Using parser As New TextFieldParser(master_path, encoding)
                With parser
                    ...
                    While Not .EndOfData
                        ...
                        ' レコードをリストに追加
                        If isHeader Then
                            HeaderFields.AddRange(.ReadFields())
                            isHeader = False
                        Else
                            FieldsList.Add(.ReadFields())
                        End If
                    End While
                End With
            End Using
        ...
        Catch ex As Exception
            Logger.Warn("err22: Verification Open Error")
            Logger.Warn("type=" & TB_Model.Text)
            Logger.Warn(ex.ToString)

            ' csvファイル読込チェックでエラーがあった場合は、エラーを表示して検証終了
            MessageBox.Show("Close " & MASTER_FILENAME & " , and Restart DNC")
        ...
        ' --- 検証データの型式毎に処理
        For Each fields As String() In FieldsList

            Dim currentModel As String = fields(1).ToString

            ' 現在の型式をDNCに転送
            Call SearchTB_Model(currentModel)

            ' 表示されたページのコントロール毎に処理
            For Each rw As DataRow In DSprg.Tables("T_cntrl" & dispPrgNo).Rows

                Dim macroString As String = rw("MACRO").ToString
                Dim sendFlg As String = rw("SENDFLG").ToString

                If macroString.Equals("add") Then
                    Continue For

                ElseIf sendFlg.Equals("1") Then

                    Dim value As String = chkSendMacro(macroString)

                    Select Case value
                        Case "err"
                            ' 現在のコントロール値にエラーが発生
                            OutputList.Add({currentModel, macroString, "is err -->", value})

                        Case "nothing"
                            ' コントロールが見つからない
                            OutputList.Add({currentModel, macroString, "is nothing -->", value})

                        Case Else

                            ' 検証データのインデックスを取得
                            Dim i As Integer = HeaderFields.IndexOf(macroString)

                            If i < 0 Then Continue For

                            Dim numValue As Single
                            If Not Single.TryParse(value, numValue) Then
                                OutputList.Add({currentModel, macroString, "is not numeric -->", value})
                            End If

                            Dim numCorrect As Single
                            If Not Single.TryParse(fields(i), numCorrect) Then
                                OutputList.Add({currentModel, macroString, fields(i).ToString, "<-- is not numeric"})
                            End If

                            ' 検証データと現在のコントロールの値を比較
                            If Not numValue.Equals(numCorrect) Then

                                ' 型式、現在のマクロ番号、検証データ、現在のコントロール値
                                OutputList.Add({currentModel, macroString, fields(i).ToString, value})
                            End If
                    End Select

                ElseIf sendFlg.Equals("2") Then
                ElseIf sendFlg.Equals("3") Then
                End If
            Next
        Next
        ...
        ' --- 検証結果出力
        Dim output_path As String = System.IO.Path.Combine(Me.Path, OUTPUT_FILENAME)
        Dim append As Boolean = False

        Try
            Using sw As New System.IO.StreamWriter(output_path, append, encoding)

                sw.WriteLine(String.Join(",", {"型式", "マクロ番号", "正答", "出力"}))

                For Each fields As String() In OutputList
                    sw.WriteLine(String.Join(",", fields))
                Next
            End Using

        Catch ex As Exception
            Logger.Warn("err23: Verification Output Error")
            Logger.Warn("type=verification output")
            Logger.Warn(ex.ToString)

            MessageBox.Show("Verification Output Error")
            isVerification = False
            Exit Sub
        End Try

        ' --- メッセージ出力
        If OutputList.Count.Equals(0) Then
            MessageBox.Show("ＯＫ！　不具合はありませんでした。")
        Else
            MessageBox.Show("ＮＧ！　Verification_result.csv を確認して下さい。")
        End If
...
#Region "プロセス間通信受信機能"
    '200423 oda プロセス間通信受信機能 -->
    Private Sub ConnectionChange(ByVal sender As Object, e As EventArgs) Handles PipeClient.ConnectionChange
        CB_Connection.Checked = PipeClient.IsConnected
    End Sub

    Private Sub ReceiveData(ByVal sender As Object, e As EventArgs) Handles PipeClient.ReceivedData
        TB_Barcode.Text = PipeClient.Text
        IsReceived = True
        Call TB_Barcode_KeyDown(Me, New KeyEventArgs(New Keys))
    End Sub
